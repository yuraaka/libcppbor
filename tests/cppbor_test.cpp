...
TEST(ConvertTest, ViewTstr) {
    ...
    EXPECT_EQ("hello"sv, item->asViewTstr()->value());
}

TEST(ConvertTest, ViewBstr) {
    unique_ptr<Item> item = details::makeItem(ViewBstr("\x23\x24\x22"));
    ...
    EXPECT_EQ("\x23\x24\x22"sv, item->asViewBstr()->value());
}
...
TEST(CloningTest, ViewBstr) {
    ViewBstr item("\x01\x02\x03\xff\x00");
    auto clone = item.clone();
    EXPECT_EQ(clone->type(), BSTR);
    EXPECT_NE(clone->asViewBstr(), nullptr);
    EXPECT_EQ(item, *clone->asViewBstr());
    EXPECT_EQ(*clone->asViewBstr(), ViewBstr("\x01\x02\x03\xff\x00"));
}
...
TEST(FullParserTest, ViewTstr) {
    ...
    auto [item, pos, message] = parseWithViews(val.encode());
    EXPECT_THAT(item, MatchesItem(val));
}

TEST(FullParserTest, ViewBstr) {
    ViewBstr val("\x00\x01\0x02"s);
