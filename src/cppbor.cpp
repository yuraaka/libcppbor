...
bool prettyPrintInternal(const Item* item, string& out, size_t indent, size_t maxBStrSize,
    ...
    case SEMANTIC: {
        ...
        out.append(buf);
        prettyPrintInternal(semantic->child().get(), out, indent, maxBStrSize, mapKeysToNotPrint);
    } break;
...
size_t headerSize(uint64_t addlInfo) {
    if (addlInfo < ONE_BYTE_LENGTH)
        return 1;
    if (addlInfo <= std::numeric_limits<uint8_t>::max())
        return 2;
    if (addlInfo <= std::numeric_limits<uint16_t>::max())
        return 3;
    if (addlInfo <= std::numeric_limits<uint32_t>::max())
        return 5;
    return 9;
}
...
uint8_t* encodeHeader(MajorType type, uint64_t addlInfo, uint8_t* pos, const uint8_t* end) {
    ...
    if (end - pos < static_cast<ssize_t>(sz))
        return nullptr;
    switch (sz) {
    case 1:
...
uint8_t* Bstr::encode(uint8_t* pos, const uint8_t* end) const {
    ...
    if (!pos || end - pos < static_cast<ptrdiff_t>(mValue.size()))
        return nullptr;
...
void Map::assertInvariant() const { CHECK(mEntries.size() % 2 == 0); }
...
void Semantic::assertInvariant() const { CHECK(mEntries.size() == 1); }
